...
export default class DocViewer extends Component {
  ...
  */
  handlePressLocal = () => {
    this.setState({animating: true});
    if(Platform.OS === 'ios'){
        OpenFile.openDoc([{url:"https://cdn.hashnode.com/res/hashnode/image/upload/w_240,h_240,c_thumb/jxjawrqbrtcjs9fjchwm/1501307390.jpg",
        fileNameOptional:"monkey.jpg",
        fileType:"jpg"
      }], (error, url) => {
         if (error) {
          this.setState({animating: false});
    ...
    handlePressLocalXLS = () => {
      this.setState({animating: true});
      if(Platform.OS === 'ios'){
          OpenFile.openDoc([{url:"https://www.cmu.edu/blackboard/files/evaluate/tests-example.xls",
          fileNameOptional:"example.xls"
        }], (error, url) => {
           if (error) {
            this.setState({animating: false});
           } else {
  ...
  render() {
    ...
    return (
      ...
      <ActivityIndicator
        ...
        />
        <Button
          onPress={this.handlePressPdf.bind(this)}
          title="document.pdf"
          accessibilityLabel="See a Document"
        />
        <Button
        ...
        <Button
          ...
          title="binaryFile.dat"
          accessibilityLabel="See a Document"
        />
        <Button
          onPress={this.handlePressb64.bind(this)}
          title="base64File.b64"
          accessibilityLabel="See a Document"
        />
        <Button
          onPress={()=>this.handlePressVideo("http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerFun.mp4")}
          title="sample.mp4"
          accessibilityLabel="See a Document"
        />
      </View>
